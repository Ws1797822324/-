...
- (UITableView *)searchtableView{
    if (!_searchtableView) {
        ...
        [_searchtableView registerNib:[UINib nibWithNibName:@"HomeCell" bundle:[NSBundle mainBundle]] forCellReuseIdentifier:@"HomeCell_ID"];
        [_searchtableView registerNib:[UINib nibWithNibName:@"AllPeopleTableViewCell" bundle:[NSBundle mainBundle]] forCellReuseIdentifier:@"AllPeopleTableViewCell"];
        if ([self.type isEqualToString:@"人"]) {
            _searchtableView.mj_footer = [MJRefreshBackNormalFooter footerWithRefreshingTarget:self refreshingAction:@selector(loadMorePeopleData)];
        }else{
...
- (void)viewDidLoad {
    ...
    [btn addTarget:self action:@selector(deleAllClick) forControlEvents:UIControlEventTouchUpInside];
...
- (void)saveStringWith:(NSString *)string
{
    ...
    BOOL isyes = [[XXHelper arrayWithMemberIsOnly:_dataArray] writeToFile:documentPath atomically:YES];
...
- (UIView *)tipView
{
    if (!_tipView) {
        ...
        UILabel * label = [[UILabel alloc] initWithFrame:CGRectMake(0, (kHeight-80)/2, kWidth, 40)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell) {
        cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:@"hjhjcell"];
    }
    ...
    if (tableView==self.tableView) {
        ...
        if ([self.type isEqualToString:@"房屋"]) {
            ...
            homecell.model = self.searchDataArray[indexPath.row];
            homecell.selectionStyle = UITableViewCellSelectionStyleNone;
            return homecell;
        }else{
            AllPeopleTableViewCell * peopleCell = [tableView dequeueReusableCellWithIdentifier:@"AllPeopleTableViewCell"];
            PeopleListModel * model = self.searchDataArray[indexPath.row];
            peopleCell.titleLabel.text = [NSString stringWithFormat:@"%@ %@",model.name,model.phone];
            peopleCell.timeLabel.text = model.time;
            return peopleCell;
        }
...
- (void)createdNavUI
{
    UIBarButtonItem * item = [[UIBarButtonItem alloc] initWithTitle:@"搜索" style:UIBarButtonItemStylePlain target:self action:@selector(searchClick)];
    self.navigationItem.rightBarButtonItem = item;

    UIView * view = [[UIView alloc] initWithFrame:CGRectMake(0, 10, kWidth - 110, 30)];
...
- (void)loadMorePeopleData
{
    ...
    kUserData;
    NSDictionary * dic = [[NSDictionary alloc] initWithObjectsAndKeys:@"my_client",@"opt",self.TF_search.text,@"name",userInfo.token,kToken,self.page,@"page",self.rows,@"row", nil];
...
{                         
    ...
    [XXNetWorkManager requestWithMethod:POST withParams:dic withUrlString:@"Houses" withHud:nil withProgressBlock:^(float requestProgress) {
