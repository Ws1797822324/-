...
- (UIButton *)leftBtn {
    if (!_leftBtn) {
        ...
        _leftBtn.layer.borderColor = RGB_HEX(0x66a8fc, 1.0).CGColor;
        _leftBtn.layer.borderWidth = 1.0f;
        _leftBtn.layer.masksToBounds = YES;
        _leftBtn.titleLabel.font = [UIFont systemFontOfSize:15.0f];
        [_leftBtn setTitleColor:RGB_HEX(0x66a8fc, 1.0) forState:UIControlStateNormal];
...
- (UIButton *)rightBtn {
    if (!_rightBtn) {
        ...
        _rightBtn.backgroundColor = RGB_HEX(0x66a8fc, 1.0);
...
- (UILabel *)titleLabel {
    if (!_titleLabel) {
        ...
        _titleLabel.textColor = RGB_HEX(0x66a8fc, 1.0);
...
- (UIView *)lineView {
    if (!_lineView) {
        _lineView = [[UIView alloc]initWithFrame:CGRectMake(0, kTopViewHeight, SCREEN_WIDTH, 0.8)];
