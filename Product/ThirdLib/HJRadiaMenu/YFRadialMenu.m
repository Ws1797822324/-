...
//        self.centerView.frame = CGRectMake(self.bounds.origin.x, self.bounds.origin.y-50, self.bounds.size.width, self.bounds.size.height);
        self.startAngle = -90-45+10;
        self.distanceBetweenPopouts = 50;
        self.distanceFromCenter = 110;
        self.stagger = 0.06;
...
- (UIView *) makeDefaultPopupView {
    UIView *view = [UIView new];
    view.frame = CGRectMake(0, 0, self.frame.size.width-20, self.frame.size.height-20 );
...
//    view.layer.shadowOffset = CGSizeMake(0, 3);
    ...
    kViewRadius(imgView, view.frame.size.height/2);
