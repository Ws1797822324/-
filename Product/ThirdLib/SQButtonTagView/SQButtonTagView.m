...
+ (CGFloat)returnViewHeightWithTagTexts:(NSArray *)tagTexts
    ...
    if (eachNum > 0) {
        ...
        for (NSInteger i = 0; i < tagTexts.count; i++) {
            ...
            CGFloat itemWidth = [[self alloc] sizeForText:tempString
                                                     Font:tagTextFont
                                                     size:CGSizeMake(MAXFLOAT, tagHeight)
...
- (void)refreshView {
    ...
    for (UIButton *buttonTag in self.buttonTags) {
        if ([self.selectArray containsObject:@(buttonTag.tag - 101)]) {
            ...
            [buttonTag setBackgroundColor:kRGB_HEX(0xf3f3f3)];
...
- (id)initWithTotalTagsNum:(NSInteger)totalTagsNum
    ...
    if (self = [super init]) {
        ...
        for (NSInteger i = 0; i < totalTagsNum; i++) {
            ...
            if (type == 3) {
                XXDeleteButton *button = [XXDeleteButton buttonWithType:UIButtonTypeCustom];
                ...
                [button addTarget:self
                              action:@selector(buttonTypeAction:)
