...
+ (void) requestWithMethod:(HTTPMethod)method withParams:(id)params withUrlString:(NSString *)urlStr withHud:(NSString *)hudStr withProgressBlock:(requestProgress)progress withSuccessBlock:(successBlock)success withFailuerBlock:(failuerBlock)failuer {
    NSString *URLString;

    if (urlStr.length>30)  {
        URLString = urlStr;
    } else {
        URLString = [NSString stringWithFormat:@"%@%@",kBaseURL,urlStr];
    }

        if (!kStringIsEmpty(hudStr)) {
        [XXProgressHUD showLoading:hudStr];
    }
    switch (method) {
        case GET:
        {
            [[XXNetWorkManager sharedManager] GET:URLString parameters:params progress:^(NSProgress * _Nonnull downloadProgress) {
                
            } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
                

                NSLog(@"请求成功 URLStr \n%@ \n --- %@\n\n",task.currentRequest.URL,params);

                success(responseObject, [responseObject[@"code"] intValue],responseObject[@"message"],responseObject[@"data"]);
        ...
        case POST:
        {
            [[XXNetWorkManager sharedManager] POST:URLString parameters:params progress:^(NSProgress * _Nonnull uploadProgress) {
...
+(void)uploadImageWithOperations:(NSDictionary *)operations withImageArray:(NSArray *)imageArray withtargetWidth:(CGFloat )defineWidth withUrlString:(NSString *)urlString withSuccessBlock:(successBlock)successBlock withFailurBlock:(failuerBlock)failureBlock withUpLoadProgress:(uploadProgress)progress;
{
    ...
    [[XXNetWorkManager sharedManager] POST:urlString parameters:operations constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        successBlock(responseObject, [responseObject[@"code"] intValue],responseObject[@"message"],responseObject[@"data"]);
...
+(void) uploadFileWorkwithParams:(id)params withUrlString:(NSString *)urlStr withHud:(NSString *)hudStr withProgressBlock:(requestProgress)progress withSuccessBlock:(successBlock)success withFailuerBlock:(failuerBlock)failuer  {
    [[XXNetWorkManager sharedManager] POST:urlStr parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
